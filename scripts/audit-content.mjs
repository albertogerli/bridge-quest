...
const folder = index > -1 ? process.argv[index + 1] : 'tmp/editorial-audit';
if (!folder) throw new Error('Missing --out folder');
const out = pathToFileURL(resolve(folder) + '/');
...
const columns = {
  ...
  glossary: 'id,term,definition,example,cards,quiz,quiz_en,term_en,definition_en,example_en',
...
const compiled = buildSync({ entryPoints: ['src/lib/content-validation.ts'], bundle: true, platform: 'node', format: 'cjs', write: false });
const module = { exports: {} };
new Function('module', compiled.outputFiles[0].text)(module);
const { validateContentBlocks, hasTranslationArtifact } = module.exports;
let data;
...
function inspectBlocks(rows, table) {
  for(const row of rows) {
    ...
    for(const lang of ['content','content_en']) {
      ...
      for (const e of validateContentBlocks(contents)) issue('ui-' + e.code, `${table}/${row.module_id??row.id}/${lang}/${e.block}`, {});
...
for(const g of data.glossary) {
  if ([g.term_en, g.definition_en, g.example_en, g.quiz_en?.question, g.quiz_en?.explanation].some(s => typeof s === 'string' && hasTranslationArtifact(s))) issue('ui-translation-artifact', `glossary/${g.id}`, {});
  ...
  if(!g.quiz_en || g.quiz_en.correctAnswer !== g.quiz.correctAnswer || g.quiz_en.options?.length !== g.quiz.options?.length) issue('glossary-translation',`glossary/${g.id}`,{});
}
...
// Structural failures block publication; semantic candidates still require a reviewer.
const blocking = summary.issues.filter(i => i.kind.startsWith('ui-') || ['answer-index', 'deal-hand-size', 'deal-card-integrity', 'invalid-card', 'invalid-lead', 'auction-contract-mismatch', 'auction-declarer-mismatch', 'translation-structural-drift'].includes(i.kind));
console.log(JSON.stringify({ blocking: blocking.length, semanticReview: summary.issues.length - blocking.length }));
if (process.argv.includes('--check') && blocking.length) process.exitCode = 1;
