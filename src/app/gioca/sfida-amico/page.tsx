...
function ActiveChallenge({
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    const link = `https://bridgelab.it/gioca/sfida-amico?id=${encodeURIComponent(challengeCode)}`;
    try {
...
function CopyMiniButton({ challengeId }: { challengeId: string }) {
  ...
  const handleCopy = async () => {
    const link = `https://bridgelab.it/gioca/sfida-amico?id=${encodeURIComponent(challengeId)}`;
    try {
      await navigator.clipboard.writeText(link);
      setCopied(true);
